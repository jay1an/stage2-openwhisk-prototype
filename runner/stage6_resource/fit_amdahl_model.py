...
DEFAULT_TRACE = "reports/civic_memory_cpu_sweep_4tier_10cycle/trace.csv"
DEFAULT_WORKFLOW = "configs/civic_alert_flow.yaml"
DEFAULT_OUT_DIR = "reports/stage6_amdahl_model"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fit per-stage Amdahl-style resource-latency models."
    )
...
def prepare_trace(path: Path) -> pd.DataFrame:
    ...
    frame = frame.dropna(subset=["allocated_memory_mb", "allocated_cpu_cores", "action_duration_ms"])
    return frame
...
def worker_breakpoints(warm_rows: pd.DataFrame, stage_spec: dict[str, float]) -> pd.DataFrame:
    ...
    grouped["worker_cap"] = grouped["parallel_workers_used"].round().clip(lower=1, upper=max_workers)
    grouped["parallel_denom"] = np.minimum(grouped["allocated_cpu_cores"], grouped["worker_cap"])
...
def residual_percent(actual: np.ndarray, predicted: np.ndarray) -> tuple[float, float]:
...
def fit_amdahl(means: pd.DataFrame, stage_spec: dict[str, float]) -> tuple[np.ndarray, np.ndarray]:
    ...
    result = least_squares(objective, initial, bounds=(0.0, np.inf), max_nfev=20_000)
...
def plot_predictions(
    ...
    for ax, stage in zip(axes, stages):
        ...
        stage_means = fit_means[fit_means["stage_name"] == stage].sort_values("allocated_cpu_cores")
        arow = amdahl_params[amdahl_params["stage_name"] == stage].iloc[0]
        prow = powerlaw_params[powerlaw_params["stage_name"] == stage].iloc[0]
        cpu_line = np.linspace(stage_means["allocated_cpu_cores"].min(), stage_means["allocated_cpu_cores"].max(), 100)
        ...
        ax.scatter(stage_rows["allocated_cpu_cores"], stage_rows["action_duration_ms"], alpha=0.55, label="actual warm samples")
...
def write_readme(
    ...
    for row in amdahl_params.sort_values("stage_name").to_dict(orient="records"):
        ...
        amdahl_lines.append(
            ...
            "{cpu_iters:.0f} | {expected_serial:.0f} | {expected_parallel:.0f} |".format(
                **row,
    ...
    cold_summary = (
        cold_overhead.groupby("stage_name")["cold_overhead_mean_ms"]
        .agg(["mean", "min", "max"])
        .reset_index()
    )
    cold_lines = [
        f"| {row.stage_name} | {row['mean']:.1f} | {row['min']:.1f} | {row['max']:.1f} |"
        for _, row in cold_summary.sort_values("stage_name").iterrows()
    ]
    best_lines = [
    ...
    text = """# Stage 6 Amdahl Resource Model

The fit uses only warm action executions from the civic memory/CPU sweep. Each
stage is fit against the four per-tier mean `action_duration_ms` values.

## Amdahl Parameters

| stage | S_ms | P_ms | C_ms | RMS error | cpu_iters | expected serial iters | expected parallel iters |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
{amdahl_rows}

`S_ms + P_ms` generally follows the YAML `cpu_iters` ordering: stages with larger
configured CPU work such as `match_face` have larger fitted CPU-work terms.
...
| stage | mean ms | min tier mean ms | max tier mean ms |
| --- | ---: | ---: | ---: |
{cold_rows}

Cold overhead is much less CPU-scaled than warm action duration. The per-tier
means stay in the same second-scale band, while warm action execution drops
monotonically with higher CPU tiers.
""".format(
        amdahl_rows="\n".join(amdahl_lines),
        best_rows="\n".join(best_lines),
        cold_rows="\n".join(cold_lines),
...
def main() -> None:
    ...
    for stage in sorted(specs):
        ...
        means = (
            ...
            .agg(action_duration_ms=("action_duration_ms", "mean"), samples=("action_duration_ms", "size"))
            .merge(workers, on="allocated_cpu_cores", how="left")
            .sort_values("allocated_cpu_cores")
        )
        if len(means) != 4:
            raise ValueError(f"expected 4 tier means for {stage}, found {len(means)}")
        ...
        amdahl_max, amdahl_rms = residual_percent(actual, amdahl_pred)
        power_max, power_rms = residual_percent(actual, power_pred)
    ...
    fit_means = pd.DataFrame(mean_rows)
    ...
    write_readme(out_dir / "README.md", amdahl_params, powerlaw_params, comparison, cold_overhead, specs)

    print("Amdahl parameters:")
    print(amdahl_params.to_string(index=False))
    print("\nPower-law parameters:")
    print(powerlaw_params.to_string(index=False))
    print("\nBest model by RMS relative error:")
    best = comparison.sort_values(["stage_name", "rms_relative_error"]).groupby("stage_name", as_index=False).first()
    print(best[["stage_name", "model", "rms_relative_error"]].to_string(index=False))
    print("\nCold overhead summary:")
    print(cold_overhead.to_string(index=False))
    print(f"\nwrote {out_dir}")
